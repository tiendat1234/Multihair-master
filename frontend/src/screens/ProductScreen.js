...
function ProductScreen() {
  ...
  const addToCartHandler = async () => {
    ...
    if (data.countInStock < quantity) {
      window.alert("Xin Lỗi, Sản Phẩm Hiện Hết Hàng");
      return;
    }
    ctxDispatch({
      type: "CART_ADD_ITEM",
  ...
  const submitHandler = async (e) => {
    e.preventDefault();
    if (!comment || !rating) {
      toast.error("Vui Lòng Để Lại Đánh Giá Và Bình Luận");
      return;
    }
    try {
      ...
      toast.success("Bài Đánh Giá Được Gửi Thành Công");
        ...
        <div className="my-3">
          {userInfo ? (
            <form onSubmit={submitHandler}>
              <h2>Hãy Viết Cảm Nhận Của Bạn Về Sản Phẩm</h2>
